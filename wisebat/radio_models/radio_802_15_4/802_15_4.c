...
void tx_end(call_t *c, packet_t *packet) {
    ...
    battery_set_radio_mode(c, MODE_IDLE);
...
void cs(call_t *c, packet_t *packet) {
    ...
    /* half-duplex */
    if (nodedata->tx_busy != -1) {
        DBG_CS(printf("! tx_busy\n"));
        return;
    }

    /* check sensibility */
    if (packet->rxdBm < nodedata->mindBm) {
        DBG_CS(printf("! rxdBm not enough\n"));
      return;
    }

    /* check channel */
    if (nodedata->channel != packet->channel) {
        DBG_CS(printf("! channel\n"));
        return;
    }
    ...
    if (packet->rxdBm > nodedata->rxdBm) {
        ...
        battery_set_radio_mode(c, MODE_RX);
        return;
    }
    DBG_CS(printf("! rxdBm\n"));
