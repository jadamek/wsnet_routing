// sf.c
// Stateless Flooding + Planar Geocasting + Greedy (SF+SPG+G) Module
...
// A simple greedy forward. Returns true if the forward was successful, and
// false if the current node is a local maximum.
bool greedy_forward(call_t *call, packet_t *packet)
{
    node_data_t *node_data = NODE_DATA(call);
    header_t *header = PACKET_HEADER(packet, node_data);
