// sf.c
// Stateless Flooding Module +(Shortest Path and Connected Dominating Set)
...
int destroy(call_t *call)
{
    ...
    fprintf(stderr, "  BFS Latency: %11d nanoseconds\n",
    ...
    {
	fprintf(results, "%d\t%d\t%11d\n", entity_data->total_num_hops, entity_data->num_packets, entity_data->dijk_latency);
    }
    free(entity_data);
    entity_data = NULL;
    return 0;
...
void tx(call_t *call, packet_t *packet)
{
    ...
    //forward the packet
    fprintf(stderr, "[RTG] Flooding at %d with TTL %d.\n", my_pos.id, header->ttl);
    forward(call, packet);
    ...
    switch(header->type)
    {
        ...
        case DATA_PACKET: 
            ...
            if((result = find_mate(call, packet)) == FOUND_OTHER_1)
                break;
	    if(header->ttl <= 0){
		fprintf(stderr, "TTL reached. Possible infinite routing.");
		break;
	    }
...
#ifdef LOG_ROUTING
    PRINT_ROUTING(stderr, "[RTG] packet at %d from %d looking for mate\n", call->node,
	header->sender.id);
#endif
