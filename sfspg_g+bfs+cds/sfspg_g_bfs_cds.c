// spg.c
// Stateless Flooding + Planar Geocasting + Greedy (SF+SPG+G) Module with Dijkstra and Connected
// Dominating Set
// James Robinson
// 12/11/2016
...
model_t model = 
{
    "Greedy Stateless Flooding and Planar Geocasting with BFS and CDS",
    "Jordan Adamek",
    "0.1",
    MODELTYPE_ROUTING,
    {NULL, 0}
...
void tx(call_t *call, packet_t *packet)
{
    ...
    // Packets within the region simply flood
    if(flooding_packet(call, packet)){
	fprintf(stderr, "[RTG] Flooding packet at %d\n", call->node);
	sf_spray_packets(call, packet);
    }
    // Outer packets begin as a greedy forwarding until local minimum is reached
    else if(header->direction == NO_DIR){
	if(!greedy_forward(call, packet)){
	    // Once local minimum is reached, packet reverts to SF+SPG routing
	    header->src = my_pos;
	    traverse_first_face(call, packet);
        }
    }
    else{
	fprintf(stderr, "[RTG] Face routing packet at %d\n", call->node);
	forward(call, packet);
    }
//getchar();
    return;
...
bool flooding_packet(call_t *call, packet_t *packet){
    ...
    return check_in_geocast(&header->dest, &me);
}
