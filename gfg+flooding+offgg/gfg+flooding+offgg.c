...
model_t model = 
{
    "GFG + Stateless Flooding with BFS, CDS, and error rate",
    "Jordan Adamek",
    "0.1",
    MODELTYPE_ROUTING,
    {NULL, 0}
...
#ifdef LOG_ROUTING
        fprintf(stderr, "[RTG] GFG routing packet at %d\n", call->node);
#endif
        forward(call, packet);
    }
...
void forward(call_t *call, packet_t *packet)
{
    ...
    if(header->mode == GREEDY || (distance(&my_pos.position, &header->dest.position) < distance(&my_pos.position, &header->check_point.position))){
	if(header->mode == FACE){
#ifdef LOG_ROUTING
	...
	if(greedy_forward(call, packet)){
	    ...
	    if(distance(&my_pos.position, &leftChoice.position) < distance(&my_pos.position, &rightChoice.position)){
