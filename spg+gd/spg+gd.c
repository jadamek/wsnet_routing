// spg.c
// Stateless Planar Geocasting Module with Geocasting Dijkstra
// James Robinson
// 12/11/2016
...
model_t model = 
{
    "Stateless Planar Geocasting with Geocasting Dijkstra",
    "James Robinson",
    "0.1",
    MODELTYPE_ROUTING,
    {NULL, 0}
...
int destroy(call_t *call)
{
    ...
    fprintf(stderr, "  Dijkstra Latency: %lld nanoseconds\n",
    ...
    else if(entity_data->deliver_num_hops != 0)
    {
...
void* get_shortest_path(call_t *call, destination_t *dest)
{
    ...
    *last_found = *new;
	...
	{
	    //remove previous node from to_check if no neighbors were checked
	    if(!nodes_visited)
	    {
	...
	//dupicate check for visited neighbors for last node in list since loop
	...
	if(new->this != NONE)
	{
	    destination_t tmp = {new->this, *get_node_position(new->this)};
