...
/* ************************************************** */
model_t model =  {
    "Send data message at random type",
    "Tony Ducrocq",
    "0.1",
...
int bootstrap(call_t *c) {
  ...
  uint64_t start = get_time() + (3 * nodedata->period) + ((rand() * nodedata->period) / RAND_MAX);
...
int callmeback(call_t *c, void *args) {
  ...
  if ( rand()  < (nodedata->probability * RAND_MAX) ) { 
    if ( nodedata->destination == -1 ) {
      /* pick a random destination different than itself */    
      while ( (destination.id = (uint64_t)(((double)random() / (double)RAND_MAX) * get_node_count())) == c->node );
    } 
