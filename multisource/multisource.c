// muisource.c
// generic application that selects the correct number of source devices
// and launches a routing call from each
// Jordan Adamek
// 
...
model_t model =
{
	"Multisource Broadcast Application Module",
	"Jordan Adamek",
	"0.1",
...
int init(call_t *call, void *params)
{
    ...
    entity_data->sources = das_create();
	...
	if(!strcmp(param->key, "num_sources"))
	    get_param_integer(param->value, &(entity_data->num_sources));
	...
	entity_data->num_rx = entity_data->num_sources;
    }

    das_insert(entity_data->sources, (void*)&entity_data->source);

    if(entity_data->num_sources > 0){
    	// Generate remaining source list
        int i = 0;
    	destination_t* tmp = 0;             
	    ...
	    das_insert(entity_data->sources, (void*)tmp);
    	}
    }
...
int destroy(call_t *call)
{
	...
	fprintf(results, "%d\t%d\t%lld\t", entity_data->num_sources,
...
int bootstrap(call_t *call)
{
    ...
    if(node_present_in(entity_data->sources, &my_pos))
...
int call_back(call_t *call, void *args)
{
    ...
    destination_t num_sources = EMPTY_DESTINATION;
    ...
    num_sources.id = entity_data->num_sources;
    num_sources.position.x = entity_data->source.id;

    if(SET_HEADER(&call_down, packet, &num_sources) == ERROR)
    {
    ...
    entity_data->num_tx++;
    scheduler_add_callback(entity_data->num_sources * get_node_count() * TX_START_TIME, call,
...
void rx(call_t *call, packet_t *packet)
{
    ...
    {
	entity_data->latency = get_time() - (entity_data->num_sources *
	    get_node_count() * TX_START_TIME);
